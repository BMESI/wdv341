...
function SelectSingleEventByID($IDARG){
	try{
		$NewEvent = new Event();
		$inta = 0;
		global $Connection;
		if ($IDARG == ""){
			...
			if ( $Connection ){
				$SQLSelectStatement = $Connection->prepare("SELECT event_id, event_name, event_description, event_presenter, event_date, event_time FROM wdv341_event WHERE event_id = :event_id");
				$SQLSelectStatement->bindParam(':event_id', $IDARG);
				$SQLSelectStatement->execute();
								foreach($SQLSelectStatement->fetchall(PDO::FETCH_ASSOC) as $row) {
									$NewEvent->setEvent_ID($row['event_id']);
									$NewEvent->setEvent_name($row['event_name']);
									$NewEvent->setEvent_description($row['event_description']);
									$NewEvent->setEvent_presenter($row['event_presenter']);
									$NewEvent->setEvent_date($row['event_date']);
									$NewEvent->setEvent_time($row['event_time']);
								}// end loop	
			} // end if connection
			else{
									$NewEvent->setEvent_ID('n/a');
									$NewEvent->setEvent_name('n/a');
									$NewEvent->setEvent_description('n/a');
									$NewEvent->setEvent_presenter('n/a');
									$NewEvent->setEvent_date('n/a');
									$NewEvent->setEvent_time('n/a');
			} // end inner esle
