<?php
	require ("validationFuntions.php");
...
{ 
	...
	// assign honey pot field to var and if it exceeds 0 characters, re-direct to the the form and reject all inputs
	...
	// extra branch of "ifs" for radios and checkboxes
		...
		// assign values of mealbox checkbox array to mealday array - then update counter
			foreach ($_POST['mealbox'] as $Item=>$Value) {
             $MealDay[$CheckboxCount] =  $Value;
			 $CheckboxCount++;
			} // ends looop
	...
	RequireGuestTypeList($RegistrantTypeList);
	RequireCheckboxes($CheckboxCount);
	RequireRadios($BadgeHolder);
