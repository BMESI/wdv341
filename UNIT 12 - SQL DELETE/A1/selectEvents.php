<?php
	//$EventModel = "../../UNIT 12 - SQL DELETE/Event.php";	
	$SelectModel = "../../UNIT 12 - SQL DELETE/Select.php";
	//if( file_exists($EventModel) && is_readable($EventModel) ){
	//&& file_exists($SelectModel) && is_readable($SelectModel)){
	//include_once  $EventModel;
	include_once  $SelectModel;
	$NewEvent = new Event();
	$LoadEvents = new Select();			
	$Count =  sizeof($LoadEvents->SelectFormatEvents());
?>
<!DOCTYPE html>
<html >
<head>
...
<form name="form" method="post" action="selectEvents.php">
 <h3>  <?php  echo  "<span class='highlight'>" . $Count . "</span>"; ?> Events are available today. Click "select" button to view.</h3>
